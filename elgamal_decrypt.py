...
for n in range(2,21):
    ...
    print("THEGKeygen time cost:",(time.time()-starttime))
    ...
    endtime=(time.time()-starttime)/len(EK);
    print("elgamal decrypt cost:",endtime/100 )
    ...
    for h in range(0,100):
          for i in Kp:    
              ...
              W = add(W, tmp)
    endtime=(time.time()-starttime)/100;
    
    print("data user obtain data owner's secret", add(C['C1'],neg(W))==m)    
    print("THEGDecrypt time cost:",endtime)
"""

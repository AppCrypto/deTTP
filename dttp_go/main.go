...
func main() {
	...
	//编译合约（合约名称）
	go_contract.Compile(contract_name)
	...
	value := big.NewInt(0)
	auth1 := go_contract.New_auth(client, privatekey, chainID, gasLimit, value)

	//部署合约（服务器，区块链ID，合约名称，私钥）
	address := go_contract.Deploy(client, chainID, contract_name, auth1)

	//将合约变为go语言接口
	go_contract.Abigen(contract_name)
	...
	gasLimit = uint64(3000000)
	value = big.NewInt(0)
	auth2 := go_contract.New_auth(client, privatekey, chainID, gasLimit, value)
